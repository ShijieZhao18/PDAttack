...
def diffattack(
    ...
    prompt = [descriptions[label.item()].strip() + " " + target_prompt] * 2
    ...
    for _, _ in enumerate(pbar):
        ...
        def dynamic_weighted_fusion(before_attention_map, after_attention_map):
            norm_before = torch.norm(before_attention_map, p=2, dim=-1, keepdim=True)
            norm_after = torch.norm(after_attention_map, p=2, dim=-1, keepdim=True)
            alpha = norm_before / (norm_before + norm_after + 1e-6)
            combined_attention_map = alpha * before_attention_map + (1 - alpha) * after_attention_map
            return combined_attention_map

        """
                ==========================================
                === Cross-Iteration Fusion Scheme ========
                ==========================================
        """
        after_attention_map = dynamic_weighted_fusion(before_attention_map, after_attention_map)
        ...
        # Secondary Deception: Cross-Iteration Fusion Scheme for Transferability Enhancement
        attack_loss = - cross_entro(pred, label) * args.attack_loss_weight

        # Primary Deception: Latent Space Adversarial Perturbation
        variance_cross_attn_loss = after_true_label_attention_map.var() * args.cross_attn_loss_weight

        # Self-Attention Scheme for Imperceptibility Preservation
